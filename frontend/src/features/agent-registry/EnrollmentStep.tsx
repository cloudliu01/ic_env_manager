...
export function EnrollmentStep({ job, onCancel, onRetry }: { job: EnrollmentJob; onCancel: () => void; onRetry: () => void }) {
  ...
  return <section className="detail-panel" aria-live="polite"><h2>{label}</h2>
    ...
    {job.state === 'failed' || job.state === 'expired' || job.state === 'cancelled' ? <><p role="alert">Enrollment could not complete ({job.last_error_code ?? `agent_enrollment_${job.state}`}). Review the failed phase and connection details.</p><button type="button" onClick={onRetry}>Retry enrollment</button></> : null}
  </section>;
}
