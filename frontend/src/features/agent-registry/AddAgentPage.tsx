...
export function AddAgentPage() {
  ...
  return <section className="feature-page"><header className="page-header"><h1 tabIndex={-1}>Add agent</h1><p>Enroll an Agent with the Manager’s existing safe v2 flow.</p></header>
    {selectedProfile?.warning === 'trusted_lan_http_unencrypted' ? <p className="transport-alert" role="alert">Trusted-LAN connection is unencrypted. Verify this Agent remains on the trusted LAN.</p> : null}
    ...
    <ConnectionStep values={values} errors={errors} onChange={change} onBlur={blur} profiles={profiles} locked={submitting} targetLocked={Boolean(enrollmentId && !job.data && !job.isError)} />
