...
export function ConnectionStep({ values, errors, onChange, onBlur, profiles, locked, targetLocked, showSsh = true }: { values: ConnectionValues; errors: Partial<Record<keyof ConnectionValues, string>>; onChange: (field: keyof ConnectionValues, value: string) => void; onBlur: (field: keyof ConnectionValues) => void; profiles: TransportProfileOption[]; locked?: boolean; targetLocked?: boolean; showSsh?: boolean }) {
  ...
  return <fieldset className="form-stack" disabled={locked}><legend>Step 1 of 3: Connection</legend>
    ...
    {showSsh ? <>{input('sshUser', 'SSH user', 'An existing Linux user allowed to run the fixed enrollment helper.')}{input('sshHost', 'SSH host', 'The SSH address for the same Agent host.')}{input('sshPort', 'SSH port', 'The SSH port; defaults to 22.', 'number')}</> : null}
  </fieldset>;
}
