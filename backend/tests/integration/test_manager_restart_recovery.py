...
def _process_identity(pid: int) -> str | None:
    result = subprocess.run(
        ["/bin/ps", "-ww", "-o", "uid=", "-o", "lstart=", "-o", "command=", "-p", str(pid)],
        capture_output=True,
        text=True,
    )
...
def _process_ids_with_argument(argument: Path) -> set[int]:
    result = subprocess.run(
    ...
    for line in result.stdout.splitlines():
        ...
        if str(argument) in command.split():
            matches.add(int(pid))
    return matches
...
def _write_term_resistant_frontend_listener(executable: Path, dev_dir: Path) -> None:
    executable.write_text(
        f"#!{sys.executable}\n"
        "import os\n"
        "import signal\n"
        ...
        "from pathlib import Path\n"
        "port = int(sys.argv[sys.argv.index('--port') + 1])\n"
        f"term = Path({str(dev_dir / 'frontend-child.term')!r})\n"
        "signal.signal(signal.SIGTERM, lambda *_: term.touch())\n"
        "listener = socket.socket()\n"
        "listener.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)\n"
        "listener.bind(('127.0.0.1', port))\n"
        "listener.listen()\n"
        f"Path({str(dev_dir / 'frontend-child.pid')!r}).write_text(str(os.getpid()))\n"
        f"Path({str(dev_dir / 'frontend-child.ready')!r}).touch()\n"
        "os.close(1)\n"
        "os.close(2)\n"
        "while True:\n"
        "    time.sleep(0.1)\n",
...
def _install_readiness_barrier(
    executable: Path, entered: Path, release: Path
) -> None:
    executable.write_text(
        f"#!{sys.executable}\n"
        "import os\n"
        ...
        "if (\n"
        "    sys.argv[1:3] == ['-m', 'ic_env_guard.development.readiness']\n"
...
def _install_frontend_signal_probe(
    ...
    executable.write_text(
        "#!/bin/sh\n"
        f"if [ -e {str(frontend_exited)!r} ] "
        "&& { [ \"${3:-}\" = SIGTERM ] || [ \"${3:-}\" = SIGKILL ]; }; then\n"
        f"  : > {str(signal_attempted)!r}\n"
        "fi\n"
...
def _fail_frontend_metadata_publish(
    executable: Path, metadata: Path, attempted: Path, frontend_ready: Path
) -> None:
    executable.write_text(
        "#!/bin/sh\n"
        "for last do :; done\n"
        f"if [ \"$last\" = {str(metadata)!r} ]; then\n"
        f"  while [ ! -e {str(frontend_ready)!r} ]; do sleep 0.02; done\n"
        f"  : > {str(attempted)!r}\n"
        "  exit 1\n"
        "fi\n"
        "exec /bin/mv \"$@\"\n",
...
@pytest.mark.integration
def test_second_start_all_waits_for_complete_frontend_generation_publication():
    dev_dir = Path(mkdtemp(prefix="ieg-all-publish-race-", dir="/tmp")).resolve()
    dev_dir.chmod(0o700)
    environment, manager_port, agent_port = _launcher_environment(dev_dir)
    _write_frontend_listener(dev_dir / "bin" / "npm", dev_dir)
    ...
    try:
        ...
        original_pids = {
            "agent": int((dev_dir / "agent.pid").read_text()),
            "control-plane": int((dev_dir / "control-plane.pid").read_text()),
        }
        original_identities = {
            role: _process_identity(pid) for role, pid in original_pids.items()
        }
        second = _start_all(environment)
        time.sleep(2)

        assert second.poll() is None
        assert not (dev_dir / "frontend-run-1.pid").exists()
        assert all(_pid_exists(pid) for pid in original_pids.values())
        ...
        while time.monotonic() < deadline:
            if second.poll() is not None:
                ...
                raise AssertionError(second.stdout.read())
            try:
                replacement_pids = {
                    "agent": int((dev_dir / "agent.pid").read_text()),
                    "control-plane": int(
                        (dev_dir / "control-plane.pid").read_text()
                    ),
                }
                final_run = int((dev_dir / "frontend.count").read_text())
            except (FileNotFoundError, ValueError):
                time.sleep(0.02)
                continue
            final_run_path = dev_dir / f"frontend-run-{final_run}.pid"
            if (
                all(
                    replacement_pids[role] != original_pids[role]
                    for role in original_pids
                )
                and final_run_path.exists()
                and (dev_dir / "frontend.pid").exists()
                and _pid_exists(int(final_run_path.read_text()))
            ):
                break
            time.sleep(0.02)
        else:
            raise AssertionError("replacement generation did not become ready")
        _wait_for_health(f"http://127.0.0.1:{manager_port}/healthz", second)
        _wait_for_health(f"http://127.0.0.1:{agent_port}/healthz", second)
        ...
        assert first.returncode is not None
        assert second.poll() is None
        for role, pid in replacement_pids.items():
            assert _pid_exists(pid)
            identity = _process_identity(pid)
            assert identity is not None
            assert str(dev_dir / f"{role}.yaml") in identity.split()
            assert identity != original_identities[role]
        for role, pid in original_pids.items():
            assert _process_identity(pid) != original_identities[role]
        metadata_pid = int((dev_dir / "frontend.pid").read_text().split()[0])
        assert _pid_exists(metadata_pid)
        assert _process_ids_with_argument(dev_dir / "frontend-runtime") == {
            metadata_pid
        }
        final_frontend_pid = int(
            (dev_dir / f"frontend-run-{final_run}.pid").read_text()
        )
        assert _pid_exists(final_frontend_pid)
        for run in range(1, final_run):
            assert not _pid_exists(int((dev_dir / f"frontend-run-{run}.pid").read_text()))
    finally:
        release_readiness.touch()
        if first.poll() is None:
            _stop_all(first)
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            try:
                ...
                frontend_runtime_pids = _process_ids_with_argument(
            ...
            if (
                ...
                and frontend_runtime_pids == {metadata_pid}
                and _pid_exists(final_frontend_pid)
            ):
                break
            time.sleep(0.02)
        else:
...
@pytest.mark.integration
def test_term_launcher_kills_frontend_group_when_supervisor_is_stopped():
    dev_dir = Path(mkdtemp(prefix="ieg-all-supervisor-stop-", dir="/tmp")).resolve()
    dev_dir.chmod(0o700)
    environment, manager_port, agent_port = _launcher_environment(dev_dir)
    _write_term_resistant_frontend_listener(dev_dir / "bin" / "npm", dev_dir)
    process = _start_all(environment)
    supervisor_pid = 0
    child_pid = 0
    try:
        _wait_for_health(f"http://127.0.0.1:{manager_port}/healthz", process)
        _wait_for_health(f"http://127.0.0.1:{agent_port}/healthz", process)
        _wait_for_file(dev_dir / "frontend-child.ready", process)
        _wait_for_file(dev_dir / "frontend.pid", process)
        supervisor_pid = int((dev_dir / "frontend.pid").read_text().split()[0])
        child_pid = int((dev_dir / "frontend-child.pid").read_text())
        ...
        if supervisor_pid and _pid_exists(supervisor_pid):
            try:
...
@pytest.mark.integration
def test_supervisor_crash_still_cleans_frontend_group():
    dev_dir = Path(mkdtemp(prefix="ieg-all-supervisor-crash-", dir="/tmp")).resolve()
    dev_dir.chmod(0o700)
    environment, manager_port, agent_port = _launcher_environment(dev_dir)
    _write_term_resistant_frontend_listener(dev_dir / "bin" / "npm", dev_dir)
    process = _start_all(environment)
...
@pytest.mark.integration
@pytest.mark.parametrize("failure", ["identity-capture", "metadata-publish"])
def test_frontend_startup_failure_cleans_unpublished_process_group(failure):
    dev_dir = Path(mkdtemp(prefix="ieg-all-frontend-publish-", dir="/tmp")).resolve()
    dev_dir.chmod(0o700)
    environment, _, _ = _launcher_environment(dev_dir)
    _write_frontend_with_descendant(dev_dir / "bin" / "npm", dev_dir)
    if failure == "identity-capture":
        ps_failure = dev_dir / "fail-frontend-ps"
        ps_failure.touch()
        _fail_frontend_ps_after_marker(dev_dir / "bin" / "ps", ps_failure)
    else:
        _fail_frontend_metadata_publish(
            dev_dir / "bin" / "mv",
            dev_dir / "frontend.pid",
            dev_dir / "frontend-publish.attempted",
            dev_dir / "frontend-descendant.pid",
        )
    process = _start_all(environment)
    descendant_pid = 0
    try:
        ...
        if descendant_pid:
            assert not _pid_exists(descendant_pid)
        assert failure == "identity-capture" or descendant_pid
        assert not (dev_dir / "frontend.pid").exists()
        if failure == "metadata-publish":
            assert (dev_dir / "frontend-publish.attempted").exists()
    finally:
        if process.poll() is None:
            _stop_all(process)
        if descendant_pid and _pid_exists(descendant_pid):
...
@pytest.mark.integration
def test_term_escalates_to_kill_for_stubborn_frontend_group():
    dev_dir = Path(mkdtemp(prefix="ieg-all-frontend-kill-", dir="/tmp")).resolve()
    dev_dir.chmod(0o700)
    environment, _, _ = _launcher_environment(dev_dir)
    term_observed = dev_dir / "frontend.term-observed"
    frontend_pid_path = dev_dir / "frontend-stubborn.pid"
    (dev_dir / "bin" / "npm").write_text(
        ...
        "import time\n"
        "from pathlib import Path\n"
        f"term_observed = Path({str(term_observed)!r})\n"
        "signal.signal(signal.SIGTERM, lambda *_: term_observed.touch())\n"
        f"Path({str(frontend_pid_path)!r}).write_text(str(os.getpid()))\n"
        "while True:\n"
        "    time.sleep(0.1)\n",
        encoding="utf-8",
    )
    ...
    try:
        _wait_for_file(dev_dir / "frontend.pid", process)
        _wait_for_file(frontend_pid_path, process)
        frontend_pid = int(frontend_pid_path.read_text())
        ...
        assert not _pid_exists(frontend_pid)
    finally:
        if process.poll() is None:
            _stop_all(process)
        if frontend_pid and _pid_exists(frontend_pid):
            try:
...
def test_failed_tracked_frontend_install_removes_final_metadata(tmp_path):
    ...
    launcher_root.mkdir()
    copy2(PROJECT_ROOT / "start.sh", launcher_root / "start.sh")
    (launcher_root / "backend").symlink_to(PROJECT_ROOT / "backend", target_is_directory=True)
    frontend_dir = launcher_root / "frontend"
    frontend_dir.mkdir()
    dev_dir = Path(mkdtemp(prefix="ieg-install-fail-", dir="/tmp")).resolve()
    dev_dir.chmod(0o700)
    environment, _, _ = _launcher_environment(dev_dir)
    environment["SKIP_INSTALL"] = "0"
    install_failed = tmp_path / "install.failed"
    dev_started = tmp_path / "dev.started"
    child_pid_path = tmp_path / "install-child.pid"
    child_ready = tmp_path / "install-child.ready"
    child_term = tmp_path / "install-child.term"
    npm = dev_dir / "bin" / "npm"
    npm.write_text(
        f"#!{sys.executable}\n"
        "import subprocess\n"
        ...
        "        'term=Path(sys.argv[1]); ready=Path(sys.argv[2]); '\n"
        "        'signal.signal(signal.SIGTERM, lambda *_: term.touch()); '\n"
    ...
    npm.chmod(0o700)
    process = subprocess.Popen(
        [str(launcher_root / "start.sh"), "all"],
        cwd=launcher_root,
        env=environment,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
    ...
    child_pid = 0
    try:
        _wait_for_file(child_pid_path, process, timeout=30)
        ...
        assert not (dev_dir / "agent.pid").exists()
        assert not (dev_dir / "control-plane.pid").exists()
        assert install_failed.exists()
        assert child_term.exists()
        assert not _pid_exists(child_pid)
        assert not dev_started.exists()
    finally:
        if process.poll() is None:
            _stop_all(process)
        if child_pid and _pid_exists(child_pid):
...
@pytest.mark.integration
def test_supervisor_boundedly_kills_stubborn_vite_after_npm_exit():
...
@pytest.mark.integration
def test_backend_generation_mismatch_does_not_signal_reused_pid(tmp_path):
    signal_attempted = tmp_path / "signal.attempted"
...
kill() {{
  touch {str(signal_attempted)!r}
  return 0
}}
if stop_recorded_process \
  {str(pid_file)!r} /tmp/agent.yaml original-generation; then
  exit 1
fi
[[ ! -e {str(signal_attempted)!r} ]]
[[ "$(cat {str(pid_file)!r})" == "123" ]]
"""
...
@pytest.mark.integration
def test_recorded_process_initial_probe_retries_transient_ps_absence(tmp_path):
    pid_file = tmp_path / "agent.pid"
    pid_file.write_text("123\n", encoding="ascii")
    pid_file.chmod(0o600)
    probes = tmp_path / "identity.probes"
    term_sent = tmp_path / "term.sent"
    script = f"""
source {str(PROJECT_ROOT / 'start.sh')!r} help >/dev/null
recorded_process_matches() {{
  local count=0
  [[ ! -e {str(probes)!r} ]] || count="$(cat {str(probes)!r})"
  count=$((count + 1))
  printf '%s\n' "$count" > {str(probes)!r}
  if [[ "$count" == "1" ]]; then
    return 2
...
kill() {{
  [[ "$1" != "-TERM" ]] || : > {str(term_sent)!r}
  return 0
}}
wait_for_recorded_process_exit() {{ return 0; }}
sleep() {{ :; }}
stop_recorded_process \
  {str(pid_file)!r} /tmp/agent.yaml original-generation
"""
...
def test_recorded_process_exit_wait_retries_transient_ps_absence(
    tmp_path, outcome, expected_status
):
    identity_probes = tmp_path / "identity-probes"
    existence_probes = tmp_path / "existence-probes"
    script = f"""
source {str(PROJECT_ROOT / 'start.sh')!r} help >/dev/null
recorded_process_matches() {{
  local count=0
  [[ ! -e {str(identity_probes)!r} ]] \
    || count="$(cat {str(identity_probes)!r})"
  count=$((count + 1))
  printf '%s\n' "$count" > {str(identity_probes)!r}
  if [[ "$count" == "1" || {outcome!r} == "exited" ]]; then
    return 2
  fi
...
process_exists() {{
  local count=0
  [[ ! -e {str(existence_probes)!r} ]] \
    || count="$(cat {str(existence_probes)!r})"
  count=$((count + 1))
  printf '%s\n' "$count" > {str(existence_probes)!r}
  [[ "$count" == "1" ]]
}}
sleep() {{ :; }}
wait_for_recorded_process_exit 123 /tmp/agent.yaml original-generation
"""
...
@pytest.mark.integration
@pytest.mark.parametrize("probe_mode", ["normal", "missing", "malformed"])
def test_nonzero_npm_exit_boundedly_kills_stubborn_descendant(tmp_path, probe_mode):
    frontend_dir = tmp_path / "frontend"
    frontend_dir.mkdir()
    ...
    npm.chmod(0o700)
    if probe_mode == "malformed":
        ps = executable_dir / "ps"
        ps.write_text("#!/bin/sh\nprintf 'malformed\\n'\n", encoding="utf-8")
        ps.chmod(0o700)
    frontend_path = (
        f"{executable_dir}:{os.environ['PATH']}"
        if probe_mode == "normal"
        else str(executable_dir)
    )
    script = f"""
source {str(PROJECT_ROOT / 'start.sh')!r} help >/dev/null
FRONTEND_DIR={str(frontend_dir)!r}
DEV_DIR={str(dev_dir)!r}
SKIP_INSTALL=1
PATH={frontend_path!r}
start_tracked_frontend &
supervisor_pid=$!
printf '%s\n' "$supervisor_pid" > {str(supervisor_pid_path)!r}
wait "$supervisor_pid"
"""
    launcher = subprocess.Popen(
        ["bash", "-c", script],
    ...
    child_pid = 0
    try:
        _wait_for_file(supervisor_pid_path, launcher)
        _wait_for_file(child_pid_path, launcher)
        supervisor_pid = int(supervisor_pid_path.read_text())
        child_pid = int(child_pid_path.read_text())

        started = time.monotonic()
        output = launcher.communicate(timeout=6)[0]

        assert time.monotonic() - started < 6
        assert launcher.returncode == 137, output
        if probe_mode == "normal":
            assert child_term.exists()
        assert not _pid_exists(child_pid)
    finally:
    ...
    try:
        ...
        keeper = subprocess.Popen(
            [
                ...
                str(dev_dir / ".start-all.lock"),
                str(lock_ready),
            ]
        )
        _wait_for_file(lock_ready, keeper)
...
@pytest.mark.integration
def test_naturally_reaped_frontend_is_not_signaled_by_cached_pid():
    dev_dir = Path(mkdtemp(prefix="ieg-all-reaped-", dir="/tmp")).resolve()
    dev_dir.chmod(0o700)
    environment, _, _ = _launcher_environment(dev_dir)
    frontend_exited = dev_dir / "frontend.exited"
    signal_attempted = dev_dir / "frontend.signal-attempted"
    _install_frontend_signal_probe(
        dev_dir / "bin" / "python", frontend_exited, signal_attempted
    )
    (dev_dir / "bin" / "npm").write_text(
        f"#!{sys.executable}\n"
        "import time\n"
        ...
        "    time.sleep(0.02)\n"
        f"Path({str(frontend_exited)!r}).touch()\n",
        encoding="utf-8",
    )
    (dev_dir / "bin" / "npm").chmod(0o700)
    process = _start_all(environment)
    try:
        ...
        assert process.returncode == 0, output
        assert frontend_exited.exists()
        assert not signal_attempted.exists()
        assert not (dev_dir / "frontend.pid").exists()
    finally:
        if process.poll() is None:
            _stop_all(process)
...
@pytest.mark.integration
@pytest.mark.parametrize("unsafe_metadata", ["malformed", "symlink"])
def test_start_all_rejects_unsafe_frontend_metadata_without_following_it(
    unsafe_metadata,
):
    dev_dir = Path(mkdtemp(prefix="ieg-unsafe-frontend-", dir="/tmp")).resolve()
    dev_dir.chmod(0o700)
    environment, _, _ = _launcher_environment(dev_dir)
    metadata = dev_dir / "frontend.pid"
    target = dev_dir / "metadata-target"
    if unsafe_metadata == "malformed":
...
@pytest.mark.integration
@pytest.mark.parametrize("change_stage", ["during-wait", "before-kill"])
def test_post_term_generation_change_is_not_killed_and_port_preflight_fails(
    tmp_path, change_stage
):
    pid_file = tmp_path / "agent.pid"
    pid_file.write_text("123\n", encoding="ascii")
    pid_file.chmod(0o600)
    config_path = tmp_path / "agent.yaml"
    stopped = tmp_path / "generation.stopped"
    killed = tmp_path / "kill.attempted"
    identity_probes = tmp_path / "identity.probes"
    executable_dir = tmp_path / "bin"
    executable_dir.mkdir()
    (executable_dir / "python").symlink_to(Path(sys.executable))
...
recorded_process_matches() {{
  ...
  count=$((count + 1))
  printf '%s\n' "$count" > {str(identity_probes)!r}
  if [[ {change_stage!r} == "before-kill" && "$count" -ge 3 ]]; then
    printf 'replacement-generation\n'
  else
    printf 'original-generation\n'
  fi
...
wait_for_recorded_process_exit() {{
  [[ {change_stage!r} == "during-wait" ]] && return 2
  return 1
}}
kill() {{
  if [[ "$1" == "-KILL" ]]; then
...
process_exists() {{ return 0; }}
stop_recorded_process {str(pid_file)!r} {str(config_path)!r} || exit 10
: > {str(stopped)!r}
[[ ! -e {str(pid_file)!r} ]] || exit 11
BACKEND_HOST=127.0.0.1
AGENT_PORT={agent_port}
AGENT_INGEST_PORT={_unused_port()}
FRONTEND_HOST=127.0.0.1
FRONTEND_PORT={_unused_port()}
validate_development_ports_available {_unused_port()}
"""
    ...
    assert "development port already in use" in result.stderr


@pytest.mark.integration
@pytest.mark.parametrize("post_kill_identity", ["same", "changed"])
def test_post_kill_uncertainty_preserves_unknown_generation_only(
...
"""
    ...
    if post_kill_identity == "same":
        assert result.returncode != 0
        assert "development process did not exit" in result.stderr
...
@pytest.mark.integration
def test_owned_frontend_cleanup_stops_descendant_process_group(tmp_path):
