...
async def _dispatch(
    *,
    ...
    validation_response = httpx.Response(response.status_code, json=response.body)
    failure_category = (
        validate_response(validation_response) if validate_response is not None else None
    )
